<template>
  <v-container class="grey lighten-5 mt-6">
      ...
      <v-col cols="12">
        ...
        <v-card class="pa-4">
          ...
          <v-form @submit.prevent="addPlayer">
            <v-row>
              <v-col cols="12" sm="9">
                <v-text-field class="player-input" v-model="playerName" label="Player Name" />
              </v-col>
              <v-col cols="12" sm="3" class="d-flex align-center">
                <v-btn type="submit" block class="my-1" :color="mainColor">Add Player</v-btn>
              </v-col>
            </v-row>
          </v-form>
        </v-card>
    ...
    <v-row>
      <v-col>
        <v-text-field class="game-name-input" v-model="gameName" label="Game Name" />
        <v-btn
          :color="mainColor"
          @click="newGame"
...
export default {
  ...
  methods: {
    ...
    /* Start a new game with the current list of players.
     * This is created directly on Firestore */
    newGame() {
